...
func AddRoutes(r server.Router) error {
	...
	r.Use(session.Middleware(
		...
		cmp.Or(os.Getenv("SESSION_NAME"), "todo_session"),
	))
	...
	r.HandleFunc("GET /{$}", tasks.List)
	r.HandleFunc("POST /{$}", tasks.Create)
	r.HandleFunc("DELETE /{id}/delete", tasks.Delete)
	r.HandleFunc("PUT /{id}/complete", tasks.Complete)
